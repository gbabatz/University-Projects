...
import numpy as np
import sys
sys.path.append('myfunctions.py')
...
img = cv.imread('N1.png')                           # loading the image (colored)
# show(img,'original')
imgray = cv.cvtColor(img, cv.COLOR_BGR2GRAY)        # turning the image to grayscale
immedian = cv.medianBlur(imgray, 5)
# show(immedian, 'median_filter')
img_blur = myf.blur_3x3_full(immedian)           # blurring for a smoother threshold
# show(img_blur, 'opencv_blur')

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 2.thresholding ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

res, thres = cv.threshold(img_blur, 53, 255, cv.THRESH_BINARY)   # creating the binary image
# show(thres, 'threshold')
strel = cv.getStructuringElement(cv.MORPH_ELLIPSE, (23, 23))    # appliying opening in order to detach objects that may touch
...
img3 = cv.imread('N1.png')
cv.drawContours(img3, valid_contours, -1, (0, 255, 0), 2)
show(img3, 'valids')
cv.imwrite('N1_valids.png', img3)
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 1. OBJECT COUNTING ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# ~~~~~~~~~~~~~~~~~~~~~~~~ drawing the boxes of valid (with color blue) and invalid (with color red) cells
...
cv.drawContours(img, all_contours, -1, color)     # draw the contours as well on the original image
show(img, 'result')

myf.find_show_contours('N1.png', 3, 55, 23, True)
