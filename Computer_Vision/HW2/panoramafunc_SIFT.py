...
def crosscheckMatch(desc1, desc2):
    # simple matcher
...
def panorama_ltr4to2_sift(img1, img2, img3, img4):
    ...
    match12 = crosscheckMatch(desc1, desc2)
    match34 = crosscheckMatch(desc3, desc4)
...
# on the bareimage
def panorama_ltr2to1_sift(img1, img2, bareimage=None):
    sift = cv.xfeatures2d_SIFT.create(5000)
    ...
    img12 = cv.warpPerspective(img2, M12, (int(img2.shape[1] * 2), int(img2.shape[0] * 1.5)))
...
def panorama_method1(img1, img2, img3, img4):
    ...
    pano21 = panorama_ltr2to1_sift(img2f_margin, img1f, img2f)
    pano21 = cropping_with_margins(pano21)

    #step1 images34
    img3_margin = margin_top_left(img3)
    pano34 = panorama_ltr2to1_sift(img3_margin, img4, img3)
    ...
    panofinal_flipped = panorama_ltr2to1_sift(pano43_margin, pano21, pano43)
...
def panorama_method_basic(img1,img2,img3,img4):
    img5, img6 = panorama_ltr4to2_sift(img1, img2, img3, img4)
    img5 = crop(img5)
    img6 = crop(img6)
    img7 = panorama_ltr2to1_sift(img5, img6)
    return img7
...
# downsizing the images by a factor of 3
img1 = cv.resize(img1, (int(img1.shape[1]/3), int(img1.shape[0]/3)))
img2 = cv.resize(img2, (int(img2.shape[1]/3), int(img2.shape[0]/3)))
img3 = cv.resize(img3, (int(img3.shape[1]/3), int(img3.shape[0]/3)))
img4 = cv.resize(img4, (int(img4.shape[1]/3), int(img4.shape[0]/3)))

panorama_final = panorama_method1(img1, img2, img3, img4)
show(panorama_final, 'finalpanorama')
# cv.imwrite('final_panorama_bridge_sift.png', panorama_final)
...
# panorama_final = panorama_method1(img1, img2, img3, img4)
# show(panorama_final, 'finalpanorama')
# # cv.imwrite('final_panorama_default_sift.png', panorama_final)
