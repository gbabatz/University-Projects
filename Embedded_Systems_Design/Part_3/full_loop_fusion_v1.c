...
// lets put those on fast memory.. we access each value of u v 4 times
// 2 buffers of M/2 size = M = 0x1B8 bytes. lets try 0x200 size on memory 
...
void warhol_loop_fusion(){
  ...
  for(i=0;i<N;i++){
    if(!(i%2)){
      tmp_i = i/2; 
      //fill row buffers only when the condition is true, meaning every two rows
      for(k=0;k<M/2;k++){
